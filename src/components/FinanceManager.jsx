...
const FinanceManager = () => {
  ...
  const calculateCategoryTotal = (category) => {
    return getFilteredTransactions
      .filter(
        (t) => t.type === TRANSACTION_TYPES.EXPENSE && t.category === category
      )
      .reduce((sum, t) => sum + parseFloat(t.amount), 0);
  };
  ...
  const groupedTransactions = groupTransactions(paginatedTransactions);
  ...
  const prepareCategoryTrends = useMemo(() => {
    const trends = {};
  ...
  const [activeView, setActiveView] = useState("dashboard"); // can be 'dashboard' or 'planner'
  const [selectedMonthPlan, setSelectedMonthPlan] = useState(
    new Date().toISOString().slice(0, 7)
  );

  // Add this new method to filter transactions for the selected month
  const getMonthTransactions = (monthStr) => {
  ...
  const handleSavePlan = (plan) => {
    try {
      // Get existing plans
      const existingPlans = JSON.parse(localStorage.getItem('monthlyPlans') || '{}');
      
      // Update the plan for the current month
      existingPlans[plan.month] = {
        ...plan,
        updatedAt: new Date().toISOString()
      };
  
      // Save back to localStorage
      localStorage.setItem('monthlyPlans', JSON.stringify(existingPlans));
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {activeView === "dashboard" ? (
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {familyMembers.map((member) => (
                          <button
                            ...
                            className={`px-4 py-2 rounded-xl flex items-center justify-center gap-2 
                                transition-all duration-200 ${
                                  newTransaction.familyMember === member.name
          ...
          <MonthPlanner
            ...
            actualTransactions={getMonthTransactions(selectedMonthPlan)}
